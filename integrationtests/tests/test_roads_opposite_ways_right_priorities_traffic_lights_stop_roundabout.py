...
class TestRoadRightPriorities(unittest.TestCase):
    ...
    def test_non_priority_cars_should_not_pass_when_there_is_a_continuous_car_flow_at_their_right(self):
        ...
        s.add_node(ra)
        tl = traffic_light()
        tl.set_state1_orientations(Orientation.EAST, Orientation.WEST).with_timer(10)
        ...
        s.add_road(rp.connect(Orientation.WEST).to(exit1).with_length(2))

        s.add_road(entry2.connect(Orientation.WEST).to(ra).with_length(2))
        s.add_road(ra.connect(Orientation.EAST).to(exit2).with_length(2))

        s.add_road(entry5.connect(Orientation.NORTH).to(ra).with_length(2))
        s.add_road(ra.connect(Orientation.SOUTH).to(exit5).with_length(2))

        s.add_road(entry3.connect(Orientation.WEST).to(tl).with_length(2))
        s.add_road(tl.connect(Orientation.EAST).to(exit3).with_length(2))

        s.add_road(entry4.connect(Orientation.EAST).to(st).with_length(2))
        s.add_road(st.connect(Orientation.WEST).to(exit4).with_length(2))

        s.add_road(rp.connect(Orientation.EAST).to(ra).with_length(7))
        s.add_road(ra.connect(Orientation.WEST).to(rp).with_length(7))

        s.add_road(ra.connect(Orientation.NORTH).to(tl).with_length(7))
        s.add_road(tl.connect(Orientation.SOUTH).to(ra).with_length(7))

        s.add_road(tl.connect(Orientation.WEST).to(st).with_length(7))
        s.add_road(st.connect(Orientation.EAST).to(tl).with_length(7))

        s.add_road(st.connect(Orientation.SOUTH).to(rp).with_length(7))
        s.add_road(rp.connect(Orientation.NORTH).to(st).with_length(7))
    ...
    def test_right_priority_with_4_entries_4_exits_5_ways_per_node(self):
        ...
        s.add_road(entry3.connect(Orientation.SOUTH).to(junction).with_length(3).with_n_ways(3))
        s.add_road(entry4.connect(Orientation.EAST).to(junction).with_length(3).with_n_ways(3))
        s.add_road(junction.connect(Orientation.NORTH).to(exit1).with_length(3).with_n_ways(3))
        s.add_road(junction.connect(Orientation.WEST).to(exit2).with_length(3).with_n_ways(3))
