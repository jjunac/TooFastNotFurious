...
def main():
    ...
    s.add_road(entry1.connect(Orientation.NORTH).to(junction).with_length(7))
    s.add_road(junction.connect(Orientation.NORTH).to(exit1).with_length(7))
    s.add_road(junction.connect(Orientation.WEST).to(exit2).with_length(4))
