...
class TestRightPriority(unittest.TestCase):
    ...
    def test_should_connect_correctly_when_a_road_is_link_to_a_right_priority_junction(self):
        ...
        self.assertEqual({rp.nodes[0][0], r2.nodes[0][0]},
                         set(simulator.dependencies[(r1.nodes[0][0], rp.nodes[0][0])]))
    ...
    def test_should_go_when_there_is_right_priority_and_no_car_present(self):
        simulator = Simulator()
        rp = RightPriorityJunction(simulator,
                                   {Orientation.NORTH: (1, 0), Orientation.EAST: (1, 0), Orientation.SOUTH: (1, 0),
                                    Orientation.WEST: (0, 1)})
        r1 = Road(simulator, 1, Orientation.NORTH, 1)
        r2 = Road(simulator, 1, Orientation.WEST, 1)
        r3 = Road(simulator, 1, Orientation.WEST, 1)
    ...
    def test_should_respect_right_priority_when_there_are_3_inputs_and_an_exit_with_double_way_road(self):
        ...
        simulator = Simulator()
        rp = RightPriorityJunction(simulator,
                                   {Orientation.NORTH: (1, 0), Orientation.EAST: (1, 1), Orientation.SOUTH: (1, 0),
        ...
        r1.nodes[0][0].current_car = Car(
            Path(dijkstra_with_path(simulator.get_nodes(), r1.nodes[0][0], r4.nodes[0][0])), r1)
        r2.nodes[0][0].current_car = Car(
            Path(dijkstra_with_path(simulator.get_nodes(), r2.nodes[0][0], r4.nodes[0][0])), r2)
        r3.nodes[0][0].current_car = Car(
            Path(dijkstra_with_path(simulator.get_nodes(), r3.nodes[0][0], r4.nodes[0][0])), r3)
    ...
    def test_should_prioritize_the_car_on_the_left_of_the_exit_when_2_car_are_facing_each_others(self):
        simulator = Simulator()
        rp = RightPriorityJunction(simulator,
                                   {Orientation.NORTH: (0, 1), Orientation.EAST: (1, 0), Orientation.SOUTH: (0, 1),
    ...
    def test_should_return_start_nodes_when_get_start_is_called(self):
        simulator = Simulator()
        dictionnary = {Orientation.NORTH: (3, 2), Orientation.EAST: (3, 2), Orientation.SOUTH: (2, 3),
                       Orientation.WEST: (2, 3)}
    ...
    def test_should_create_intern_mesh_when_a_3_3_junction_is_created(self):
        simulator = Simulator()
        dictionary = {Orientation.NORTH: (3, 3), Orientation.EAST: (3, 3), Orientation.SOUTH: (3, 3),
                      Orientation.WEST: (3, 3)}
    ...
    def test_should_respect_right_priority_when_there_are_2_inputs_and_2_output_and_2_ways(self):
        ...
        entryW.nodes[0][0].current_car = Car(
            Path(dijkstra_with_path(simulator.get_nodes(), entryW.nodes[0][0], exitW.nodes[0][0])), entryW.nodes[0][0])
        entryN.nodes[0][0].current_car = Car(
            Path(dijkstra_with_path(simulator.get_nodes(), entryN.nodes[0][0], exitN.nodes[0][0])), entryW.nodes[0][0])
