...
class TestRoad(unittest.TestCase):
    ...
    def test_a_car_should_stop_when_there_is_a_car_ahead(self):
        ...
        p = Path([0] * 10)
        for i in range(4, 10):
            road.nodes[0][i].current_car = Car(p, road.nodes[0][0])
        road.nodes[0][1].current_car = Car(p, road.nodes[0][0])
    ...
    def test_a_car_should_wait_before_go_forward_when_there_is_a_car_ahead(self):
        ...
        p = Path([0] * 3)
        road.nodes[0][0].current_car = Car(p, road.nodes[0][0])
        road.nodes[0][1].current_car = Car(p, road.nodes[0][1])
        road.nodes[0][2].current_car = Car(p, road.nodes[0][2])
